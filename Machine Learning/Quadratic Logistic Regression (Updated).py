...
book = openpyxl.open('Quadratic Logistic Regression Data (Elliptic Data).xlsx',read_only=True)
sheet = book.active
cells = sheet['A1':'G50']
...
print("Больше итераций - качественнее решение, но дольше его поиск.\nВведите желаемое число итераций градиентного спуска: (Рекоменд. в пределах 1000-5000)")
n = ''
...
while iter_counter <= n-1:
    ...
    iter_counter += 1
    print(f'{iter_counter}-я итерация...')
...
#mng.window.state('zoomed') ------ БАГУЛИНА!
mng.window.showMaximized() # - верная команда (Новая?)
